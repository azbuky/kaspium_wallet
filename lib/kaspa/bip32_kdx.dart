...
const kUint32MAX = 4294967295;
const kHighestBit = 0x80000000;
class BIP32Kdx extends BIP32 {
  ...
  factory BIP32Kdx.fromPublicKey(Uint8List publicKey, Uint8List chainCode,
      [NetworkType? nw]) {
    NetworkType network = nw ?? NETWORKS.bitcoin;
  ...
  factory BIP32Kdx.fromPrivateKey(Uint8List privateKey, Uint8List chainCode,
      [NetworkType? nw]) {
    NetworkType network = nw ?? NETWORKS.bitcoin;
  ...
  BIP32 derive(int index) {
    if (index > kUint32MAX || index < 0)
      throw ArgumentError("Expected UInt32");
    final isHardened = index >= kHighestBit;
